...
class fee_type(osv.osv):
    ...
    #FIXME 该项暂不使用 最低消费:如酒水消费低于 “最低消费”则按“最低消费”收取,否则按酒水实际消费收取,其他一切费用(除服务费)均免收
    FEE_TYPE_MINIMUM_FEE = "minimum_fee"
    #FIXME 暂不使用 结账重开:只按酒水实际消费收取,其他一切费用(除服务费)均免收
    FEE_TYPE_ONLY_CONSUME_FEE = "only_consume_fee"
    #FIXME 暂不使用 包厢费+最低消费:包厢费必收,酒水费依据最低消费原则计算,除服务费外,免收其他费用
    FEE_TYPE_ROOM_FEE_PLUS_MINIMUM_FEE = "room_fee_plus_minimum_fee"
    #FIXME 暂不使用 钟点费+最低消费:钟点费必收,酒水按照最低消费计算,除服务费外,免收其他费用
    FEE_TYPE_HOURLY_FEE_PLUS_MINIMUM_FEE = "hourly_fee_plus_minimum_fee"
    #FIXME 暂不使用 包厢费+钟点费+最低消费
    ...
    _columns = {
            ...
            'service_fee_rate' : fields.float('service_fee_rate',digits_compute = dp.get_precision('ktv_fee')),
            #酒水价格类型
